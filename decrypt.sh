...
do
	...
	then
		/root/rar/rar x -ts -p"$password" "$folder.$currentdate.rar"
	fi
	if [ ! -f "$folder.$currentdate.part1.rar" ]
	then
		/root/rar/rar x -ts -p"$password" "$folder.$currentdate.part1.rar"
	fi
	if [ ! -f "$folder.$currentdate.part01.rar" ]
	then
		/root/rar/rar x -ts -p"$password" "$folder.$currentdate.part01.rar"
	fi
	if [ ! -f "$folder.$currentdate.part001.rar" ]
	then
		/root/rar/rar x -ts -p"$password" "$folder.$currentdate.part001.rar"
	fi
	cd $folder
